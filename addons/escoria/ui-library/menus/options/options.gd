...
func _ready() -> void:
	_loaded_languages = ["en","es","de", "fr"]
	_flags_container = $VBoxContainer/MarginContainer/options/flags
	pass
...
func initialize_flags():
	...
	for lang in _loaded_languages:
		var button = TextureButton.new()
		button.texture_normal = load("res://addons/escoria/assets/image/flags/%s.png" % lang)
		button.pressed.connect( self._on_language_input.bind(lang))
		_flags_container.add_child(button)
...
func _on_language_input(language: String):
...
#
##
func refresh_ui_texts(node: Node = null):
	if node == null:
		node = get_tree().root
