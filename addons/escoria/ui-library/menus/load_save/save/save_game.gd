...
func _ready():
	slot_ui_scene = load("res://addons/escoria/ui-library/menus/load_save/load_save_slot/load_save_slot.tscn")
	refresh_savegames()
...
func refresh_savegames():
	...
	new_slot.pressed.connect( self._on_slot_pressed.bind(saves_list.size()+1))
	...
	for save in saves_array:
		...
		datetime_string = "%02d/%02d/%02d %02d:%02d" % [
			save.date.get("day", 0),
			save.date.get("month", 0),
			save.date.get("year", 0),
			save.date.get("hour", 0),
			save.date.get("minute", 0),
		]

		new_slot.set_slot_name_date(save["name"], datetime_string)
		new_slot.pressed.connect( self._on_slot_pressed.bind([int(save["slotnumber"])]))
