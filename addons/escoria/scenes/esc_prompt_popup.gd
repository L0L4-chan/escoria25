...
func _ready() -> void:
	...
	escoria.logger.info(self,"pront initiate")
...
func _on_command_text_entered(p_command_str : String):
	...
	if script:
		...
		var ret = await escoria.event_manager.finished
		while ret[1] != _print.get_command_name():
			ret = await escoria.event_manager.finished
...
func _on_error_message_f(message) -> void:
