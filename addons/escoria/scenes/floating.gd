...
func _ready():
	_text_time_per_character = ESCProjectSettingsManager.get_setting(ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS)
	...
	if _text_time_per_character < 0:
		escoria.logger.warn(
			...
			"%s setting must be a non-negative number. Will use default value of %s." %
				[
					ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS,
					ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_VALUE
				]
		)

		_text_time_per_character = ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_VALUE

	_fast_text_time_per_character = ESCProjectSettingsManager.get_setting(
		ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_FAST
	)
	...
	if _fast_text_time_per_character < 0:
		escoria.logger.warn(
			...
			"%s setting must be a non-negative number. Will use default value of %s." %
				[
					ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_FAST,
					ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_FAST_VALUE
				]
		)

		_fast_text_time_per_character = ESCProjectSettingsManager.TEXT_TIME_PER_LETTER_MS_FAST_VALUE

	_reading_speed_in_wpm = ESCProjectSettingsManager.get_setting(
...
func say(character: String, line: String) :
	...
	for c in escoria.object_manager.get_object(character).node.get_children():
		if c is Marker2D:
			...
			if script and script.get_global_name() == "ESCDialogLocation" || c.is_class("ESCDialogLocation"):
	...
	if _current_character.is_inside_tree() and \
		...
		set_deferred("position:x", 0)
		set_deferred("size:x", ProjectSettings.get_setting("display/window/size/viewport_width"))
