...
func set_scene(p_scene: Node) -> void:
	...
	if not p_scene.is_inside_tree():
		...
		# room.
		# Eliminar escena previa inmediatamente
...
func set_camera_limits(camera_limit_id: int = 0, scene: Node = current_scene) -> void:
	...
	if scene_camera_limits.size.x == 0 and scene_camera_limits.size.y == 0:
		...
		if area.size.x == 0 or area.size.y == 0 or area.size <  Vector2(get_viewport().size):
			escoria.logger.warn(
				self,
	...
	escoria.player_camera.set_limits(limits)
...
func activate_camera():
	current_scene.player_camera.enabled = true
	current_scene.player_camera.make_current()
	set_camera_limits(0, current_scene)
