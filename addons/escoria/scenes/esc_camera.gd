...
# Prepare the tween 
func _ready(): 
	if (escoria.player_camera == null): 
		_tween = create_tween() 
		_tween.tween_property(self, "modulate:a", 0.0, 1.0) 
		_tween.finished.connect(_target_reached) 
		escoria.player_camera = self
		escoria.object_manager.CAMERA = self 
		register() 
	
# Update the position if the followed target is moving 
func _process(_delta): 
	if is_instance_valid(_follow_target): 
		_resolve_target_and_zoom(_follow_target)
		global_position = _convert_pos_for_disabled_drag_margin(_target) 
...
func set_target(p_target, p_time : float = 0.0): 
	...
	escoria.logger.info( self, "Current camera position = %s." % str(self.global_position) ) 
	if p_time == 0.0: 
		...
		if _tween && _tween.is_running(): 
			escoria.logger.debug( self, "set_target tween is still active: %f seconds of %f completed." % [ _tween.tell(), _tween.get_runtime() ] ) 
			_tween.kill() 
		...
		var variant : PropertyTweener = _tween.tween_property(
			self,
			"global_position",
			_target,
			p_time
		...
		variant.from(self.global_position)
		_tween.play() 
...
func set_camera_zoom(p_zoom_level: float, p_time: float): 
	if p_zoom_level <= 0.0: 
		escoria.logger.error( self, "Tried to set negative or zero zoom level." ) 
	...
	if p_time == 0: 
		...
		await get_tree().process_frame 
		if _tween && _tween.is_running(): 
			escoria.logger.debug( self, "set_camera_zoom tween is still active: %f seconds of %f completed." % [ _tween.tell(), _tween.get_runtime() ] ) 
			_tween.kill() 
		set_drag_margin_enabled(false, false) 
	_convert_current_global_pos_for_disabled_drag_margin()
	_tween = create_tween() 
...
func push(p_target, p_time: float = 0.0, p_type: int = 0): 
	...
	if p_time == 0: 
		...
		if _zoom_target != Vector2():
			...
			await get_tree().process_frame 
			if _tween && _tween.is_running(): 
				escoria.logger.debug( self, "camera push tween is still active: %f seconds of %f completed." % [ _tween.tell(), _tween.get_runtime() ] ) 
				_tween.kill() 
			if _zoom_target != Vector2(): 
				_tween.set_trans(p_type).set_ease(Tween.EASE_IN_OUT)
...
func shift(p_target: Vector2, p_time: float, p_type: int): 
	...
	if _tween && _tween.is_running(): 
		...
		# recalculated. Also to allow any close-calls with the tween to finish. 
		await (get_tree().process_frame) 
		escoria.logger.debug( self, "camera shift tween is still active: %f seconds of %f completed." % [ _tween.tell(), _tween.get_runtime() ] ) 
		_tween.kill() 
	set_drag_margin_enabled(false, false) 
	_convert_current_global_pos_for_disabled_drag_margin() 
	_tween = create_tween()
...
func check_point_is_inside_viewport_limits(point: Vector2) -> bool: 
	...
	var limits_to_test: Rect2 = Rect2( limit_left + screen_half_size.x, limit_top + screen_half_size.y, limit_right - limit_left - viewport_rect.size.x + 1, limit_bottom - limit_top - viewport_rect.size.y + 1 ) 
...
# with drag margins disabled. # # This is helpful for preventing the camera from "jumping" when disabling drag margins, e.g. in 
# order to perform some camera translations/tweening. # # #### Parameters # - pos: Position to be converted. 
