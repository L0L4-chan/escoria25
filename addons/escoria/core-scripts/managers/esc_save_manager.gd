...
func get_saves_list() -> Dictionary:
	...
	if dirsave:
		dirsave.list_dir_begin()
		var nextfile = dirsave.get_next()
		while nextfile != "":
			var save_path = save_folder + "/" + nextfile
...
func save_game_exists(id: int) -> bool:
	var save_file_path: String = save_folder + "/" + (SAVE_NAME_TEMPLATE % id)
...
func save_game(id: int, p_savename: String):
	...
	var save_path = save_folder+ "/" + (SAVE_NAME_TEMPLATE % id)
...
func save_game_crash():
	...
	crash_savegame_filename = save_file_path + "/" + (
...
func _do_save_game(p_savename: String) -> ESCSaveGame:
	if escoria == null:
		push_error("Escoria singleton not found!")
		return null
	...
	save_game.date = Time.get_date_dict_from_system()
...
func load_game(id: int):
	...
	if escoria_version != save_game.escoria_version:
		...
		save_game = migration_manager.migrate(
			...
			escoria_version,
			"res://addons/escoria/core-scripts/managers"
		)
	...
	for room_id in save_game.objects.keys():
		...
		if room_id in ESCObjectManager.RESERVED_OBJECTS:
			if save_game.objects[room_id]["state"] in ["default", "off"]:
				load_statements.append(
					ESCCommand.new("%s %s" % [
			...
			if room_id == save_game.main.last_scene_global_id:
				for object_global_id in save_game.objects[room_id].keys():
					...
					if object_global_id in save_game.inventory:
						continue				
					var obj_data = save_game.objects[room_id][object_global_id]

					if obj_data.has("active"):
						load_statements.append(
					...
					if obj_data.has("interactive"):
						load_statements.append(
							ESCCommand.new("%s %s %s" % [
								_set_interactive.get_command_name(),
								object_global_id,
								obj_data["interactive"]
							])
						)

					if obj_data.has("state") and not obj_data["state"].is_empty():
						if object_global_id != "player":
							load_statements.append(
								ESCCommand.new("%s %s %s true" % [
					...
					if obj_data.has("global_transform"):
						load_statements.append(
							ESCCommand.new("%s %s %s %s" % [
								_teleport_pos.get_command_name(),
								object_global_id,
								int(obj_data["global_transform"].origin.x),
								int(obj_data["global_transform"].origin.y)
							])
						)
						load_statements.append(
							ESCCommand.new("%s %s %s" % [
					...
					if obj_data.has("custom_data") and obj_data["custom_data"].size() > 0:
						load_statements.append(
							ESCCommand.new(
								"",
								_set_item_custom_data.get_command_name(),
								[object_global_id, obj_data["custom_data"]]
							)
						)

					if object_global_id == escoria.object_manager.CAMERA:
						camera_target_to_follow = obj_data["target"]
	...
	# FOLLOW TARGET
	if camera_target_to_follow != null and not str(camera_target_to_follow).is_empty():
		load_statements.append(
			ESCCommand.new("%s %s" % [
