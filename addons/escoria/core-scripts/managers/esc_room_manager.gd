...
func init_room(room: ESCRoom) -> void:
	...
	if room.camera_limits.size() == 1 and room.camera_limits[0].size:
		for child in room.get_children():
			if child is ESCBackground:
				room.camera_limits[0] = Rect2(0, 0, child.size.x, child.size.y)
...
func _perform_script_events(room: ESCRoom):
	...
	if room.player_scene:
		if not is_instance_valid(room.player):
			...
			for loc in room._find_esc_locations(room):
				if loc.is_start_location:
					room.player.position = loc.position
					break
	...
	if not room.player_camera:
		room.player_camera = ESCCamera.new()
		room.player_camera._follow_target = room.player
		room.add_child(room.player_camera)
		escoria.player_camera = room.player_camera
	...
	room.player_camera.enabled = false
	...
	if room.player:
		escoria.player_camera.set_target(room.player)
	...
	#room.player_camera.make_current()
	...
	# to false
	if escoria.globals_manager.get_global(escoria.room_manager.GLOBAL_FORCE_LAST_SCENE_NULL):
