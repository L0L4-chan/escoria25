...
func register_object(object: ESCObject, room: ESCRoom = null, force: bool = false, \
	...
	if object.node.is_connected("tree_exited",self.unregister_object):
		object.node.disconnect("tree_exited",self.unregister_object)
	...
	if auto_unregister:
		object.node.tree_exited.connect(self.unregister_object.bind(object, room_key))

	if "is_interactive" in object.node and object.node.is_interactive:
		object.interactive = true

	if "esc_script" in object.node and object.node.esc_script != null and object.node.esc_script != "":
...
# Actualiza el estado de un objeto ya registrado
func update_state(global_id: String, new_state: String) -> void:
	var obj = get_object(global_id)
	if obj != null:
		obj.state = new_state

		if obj.node != null and obj.node.has_method("set_state"):
			obj.node.set_state(new_state)
		escoria.logger.debug(self, "Estado de %s actualizado a '%s'" % [global_id, new_state])
	else:
		escoria.logger.warn(self, "No se encontró el objeto con global_id '%s' para actualizar su estado" % global_id)
