...
var action_state = ACTION_INPUT_STATE.AWAITING_VERB_OR_ITEM : set = set_action_input_state
	...
	if escoria.current_state == escoria.GAME_STATE.DEFAULT:
		match action:
			...
			_:
				escoria.logger.warn(
					self,
					("Action received: " + str(action) + " with params " + str(params) + "."))
...
# - destination: Marker2D or ESCObject holding the moving object to head to
...
func perform_walk(
	...
	if destination is Vector2:
		final_destination = _get_valid_destination(
				moving_obj.node.global_position,
				destination)
		...
		moving_obj.node.walk_to(final_destination, walk_context)
	# Walk to object
...
#
#This method check if the destination is accesible from the start point.
#
func _get_valid_destination(start: Vector2, destination: Vector2) -> Vector2:
	var final_destination = destination
	var walkable_area = escoria.room_terrain

	if walkable_area and walkable_area.current_active_navigation_instance:
		var nav_region = walkable_area.current_active_navigation_instance
		var nav_poly: NavigationPolygon = nav_region.navigation_polygon
		if nav_poly:
			var local_start = nav_region.to_local(start)
			var local_destination = nav_region.to_local(destination)
			var outline_index = -1
			for i in range(nav_poly.get_outline_count()):
				var outline = nav_poly.get_outline(i)
				if Geometry2D.is_point_in_polygon(local_start, outline):
					outline_index = i
					break
			if outline_index != -1:
				var outline = nav_poly.get_outline(outline_index)
				if not Geometry2D.is_point_in_polygon(local_destination, outline):
					var closest_point = outline[0]
					var min_dist = local_destination.distance_to(closest_point)
					for j in range(outline.size()):
						var s1 = outline[j]
						var s2 = outline[(j + 1) % outline.size()]
						var candidate = Geometry2D.get_closest_point_to_segment(local_destination, s1, s2)
						var dist = candidate.distance_to(local_destination)
						if dist < min_dist:
							closest_point = candidate
							min_dist = dist
					final_destination = nav_region.to_global(closest_point)
			else:
	...
	if escoria.main.current_scene.player:
		...
		if not obj.node is ESCPlayer and \
			...
			not event_flags & ESCEvent.FLAG_TK:
				...
				if interact_position != _get_valid_destination(
					escoria.main.current_scene.player.global_position,
					interact_position):
					escoria.logger.info(self, "Object can´t be reach, action cancelled.")
					clear_current_action()
					emit_signal("action_finished")
					return
...
# - clicked_position: the Marker2D of the input click
...
func _walk_towards_object(
	...
	if escoria.object_manager.get_object(obj.global_id).interactive:
		...
		dont_interact = true
	destination_position = _get_valid_destination(
		escoria.main.current_scene.player.position,
