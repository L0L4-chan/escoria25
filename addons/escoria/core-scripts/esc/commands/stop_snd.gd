...
func run(command_params: Array) -> int:
	var esc_obj = escoria.object_manager.get_object(command_params[0])
	if esc_obj:
		if esc_obj.state != null:
			previous_snd_state = esc_obj.state
		# Cambiar el estado directamente en el ESCObject
		esc_obj.node.set_state("off")
