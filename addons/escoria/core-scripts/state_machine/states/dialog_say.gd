extends "res://addons/escoria/core-scripts/state_machine/state.gd"
...
func initialize(dialog_manager: ESCDialogManager, character: String, text: String, type: String) -> void:
	...
	_text = text
	_type = type
	_stop_talking_animation_on_option = ESCProjectSettingsManager.get_setting(ESCProjectSettingsManager.STOP_TALKING_ANIMATION_ON)
...
func handle_input(_event):
	if _event is InputEventMouseButton and _event.pressed:
		if escoria.inputs_manager.input_mode != \
			...
			var left_click_action = ESCProjectSettingsManager.get_setting(ESCProjectSettingsManager.LEFT_CLICK_ACTION)
...
func enter():
	...
	if matches.get_string("key") != "":
		...
		if _speech_resource == "":
			...
			if _stop_talking_animation_on_option == ESCProjectSettingsManager.STOP_TALKING_ANIMATION_ON_END_OF_AUDIO:
				if not (
					escoria.object_manager.get_object(escoria.object_manager.SPEECH).node\
					 as ESCSpeechPlayer
		...
		while file_name != "":
			if _dir.current_is_dir():
				...
				if file_name == "%s.%s.import" % [
					...
					return start + "/" + (file_name.trim_suffix(".import"))
