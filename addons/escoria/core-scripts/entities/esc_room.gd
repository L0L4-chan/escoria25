...
@tool
@icon("res://addons/escoria/assets/image/esc_room.svg")
extends Node2D
class_name ESCRoom
...
@export var camera_limits: Array[Rect2] = []: set = set_camera_limits

# The editor debug display mode
@export var editor_debug_mode = EditorRoomDebugDisplay.NONE : set = set_editor_debug_mode



# The player scene instance
var player: ESCPlayer
...
func _draw():
	...
	for i in camera_limits.size():
		...
		draw_string(default_font, Vector2(camera_limits[i].position.x + 30,
			camera_limits[i].position.y + 30), str(i),0,-1,16, camera_limits_colors[i],3,0,0)
# orientation: TextServer.Orientation = 0) const
