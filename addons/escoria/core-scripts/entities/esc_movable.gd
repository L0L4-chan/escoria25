...
func _process(delta: float) -> void:
	...
	if task == MovableTask.WALK or task == MovableTask.SLIDE:
		...
		if task == MovableTask.WALK:
			...
			# Misma dirección pero en grados, ajustada a tu sistema (0=arriba, 90=dcha, etc.)
			var deg: int = wrapi(int(rad_to_deg(angle)) + 90, 0, 360)
			_perform_walk_orientation(deg)
...
func _perform_walk_orientation(angle: float):
	...
	last_dir = _get_dir_deg(angle, parent.animations)
...
func walk_stop(pos: Vector2) -> void:
	...
	if walk_context.target_object and \
		...
		var orientation = _get_dir_deg(walk_context.target_object.node.interaction_angle,
			parent.animations)
