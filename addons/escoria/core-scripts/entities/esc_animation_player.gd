...
func _ready() -> void:
	if _is_animation_player:
		_player_node.connect("animation_finished",self._on_animation_finished)
	else:
		_player_node.connect("animation_finished",self._on_animation_finished_animated_sprite)
...
func _on_animation_finished(name: String):
	if _is_animation_player and _animation_player.get_animation(name).loop_mode == 0:
