...
@tool
@icon("res://addons/escoria/assets/image/esc_item.svg")
extends Area2D
class_name ESCItem 
...
@export var inventory_texture: Texture : get = _get_inventory_texture

# Color used for dialogs
@export var dialog_color = Color(1,1,1,1)
...
@export var animation_player_node: NodePath = "" : set = _set_animation_player_node
...
func _ready():
	...
	if not Engine.is_editor_hint():
		...
		if is_instance_valid(escoria.room_terrain):
...
func _on_input_event(_viewport: Object, event: InputEvent, _shape_idx: int):
	if event is InputEventMouseMotion:
		...
		var colliding: Array = physics2d_dss.intersect_point(query)
		var colliding_nodes = []
		for c in colliding:
...
func _unhandled_input(input_event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.is_pressed():
		if not escoria.current_state == escoria.GAME_STATE.DEFAULT:
			#if escoria.current_state == escoria.GAME_STATE.DIALOG:
...
func _is_in_shape(position: Vector2) -> bool:
	...
	var colliders = get_world_2d().direct_space_state.intersect_point(query)
...
# item. That can either be a direct Marker2D child or a collision shape
#
...
func get_interact_position() -> Vector2:
	...
	for c in get_children():
		escoria.logger.info(self,c.get_class())
		if c is Marker2D:
			var script = c.get_script()
			if script and script.get_global_name() == "ESCLocation" || c.is_class("ESCLocation"):
				esclocation_count += 1
				esclocation_position = c.global_position
			elif script and script.get_global_name() == "ESCInteractionLocation" || c.is_class("ESCInteractionLocation"):
				interact_count += 1
				interact_position = c.global_position
			else:
				# This will catch all other Marker2D related nodes
				# including dialog locations and native Marker2D nodes.
				escoria.logger.info(self,c.get_class() )
				pos_2d_count += 1
...
func start_talking():
	...
	animation_player.play(animations.speaks[animation_direction].animation)
