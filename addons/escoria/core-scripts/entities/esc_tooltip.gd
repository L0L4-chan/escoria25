...
@export var offset_from_cursor = Vector2(10,0)

# Activates debug mode. If enabled, shows the label with a white background.
@export var debug_mode = false : set =  set_debug_mode
...
func set_debug_mode(p_debug_mode: bool):
	...
	if debug_mode:
		...
		debug_texturerect_node.texture = load("res://addons/escoria/assets/image/white.png")
