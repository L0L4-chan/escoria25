...
@export var mouse_tooltip_margin = 50.0

# Which (if any) debug mode for the editor is used
@export var editor_debug_mode = EDITOR_GAME_DEBUG_DISPLAY.NONE : set = _set_editor_debug_mode
...
func _enter_tree():
	if not escoria.event_manager.is_connected("event_finished", self._on_event_done):
		escoria.event_manager.connect("event_finished", self._on_event_done)

	if not escoria.action_manager.is_connected("action_finished", self._on_action_finished):
		escoria.action_manager.connect("action_finished", self._on_action_finished)

	if not escoria.main.is_connected("room_ready", self._on_room_ready):
	...
	if ProjectSettings.get_setting(ESCProjectSettingsManager.ENABLE_HOVER_STACK_VIEWER) and \
		...
		hover_stack_displayer = preload("res://addons/escoria/ui-library/tools/hover_stack/hover_stack.tscn").instantiate()
...
func _exit_tree():
	if escoria.event_manager.is_connected("event_finished", self._on_event_done):
		escoria.event_manager.disconnect("event_finished", self._on_event_done)

	if escoria.action_manager.is_connected("action_finished", self._on_action_finished):
		escoria.action_manager.disconnect("action_finished", self._on_action_finished)

	if escoria.main.is_connected("room_ready", self._on_room_ready):
		escoria.main.disconnect("room_ready", self._on_room_ready)
...
func show_crash_popup(files: Array = []) -> void:
	...
	crash_popup.process_mode = Node.PROCESS_MODE_PAUSABLE
