...
var state_machine = preload("res://addons/escoria/core-scripts/dialog/esc_dialog_simple_state_machine.gd").new()
...
func _init_type_player(type: String) -> void:
	if type == "floating":
		_type_player = preload("res://addons/escoria/scenes/floating.tscn").instantiate()
	else:
		_type_player = preload("res://addons/escoria/scenes/avatar.tscn").instantiate()
...
func do_choose(dialog_player: Node, dialog: ESCDialog, type: String = "simple"):
	...
	if type == "simple" or type == "":
		chooser = preload("res://addons/escoria/scenes/simple.tscn").instantiate()
...
func interrupt():
	if _type_player != null and _dialog_player.get_children().has(_type_player):
		(
